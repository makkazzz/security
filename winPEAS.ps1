...
Function Start-ACLCheck {
  ...
  if ($null -ne $target) {
    ...
    if ($ACLObject) { 
      ...
      whoami.exe /groups /fo csv | ConvertFrom-Csv | Select-Object -ExpandProperty 'グループ名' | ForEach-Object { $Identity += $_ }
      $IdentityFound = $false
...
Get-SmbShare | Get-SmbShareAccess | ForEach-Object {
  $SMBShareObject = $_
  whoami.exe /groups /fo csv | ConvertFrom-Csv | Select-Object -ExpandProperty 'グループ名' | ForEach-Object {
